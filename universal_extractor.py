...
class UniversalExtractorApp(ctk.CTk):
    ...
    def multi_engine_search(self, query, extra_variations=None):
        ...
        for variation in variations:
            try:
                with DDGS() as ddgs:
                    results = ddgs.text(variation, max_results=400)
                    for r in results:
                        urls.add(r['href'])
            except Exception as e:
        ...
        try:
            google_url = f"https://www.google.com/search?q={requests.utils.quote(query)}&num=300"
            headers = {'User-Agent': random.choice(USER_AGENTS)}
            r = requests.get(google_url, headers=headers, timeout=20)
        ...
        try:
            bing_url = f"https://www.bing.com/search?q={requests.utils.quote(query)}&count=200"
            headers = {'User-Agent': random.choice(USER_AGENTS)}
            r = requests.get(bing_url, headers=headers, timeout=20)
            if r.status_code == 200:
                soup = BeautifulSoup(r.text, 'html.parser')
                for a in soup.find_all('a', href=True):
                    href = a['href']
        ...
        try:
            yahoo_url = f"https://search.yahoo.com/search?p={requests.utils.quote(query)}&n=200"
            headers = {'User-Agent': random.choice(USER_AGENTS)}
            ...
            print(f"Yahoo query failed: {e}")

        try:
            brave_url = f"https://search.brave.com/search?q={requests.utils.quote(query)}"
            headers = {'User-Agent': random.choice(USER_AGENTS)}
            r = requests.get(brave_url, headers=headers, timeout=20)
            if r.status_code == 200:
                soup = BeautifulSoup(r.text, 'html.parser')
                for a in soup.find_all('a', href=True):
                    href = a['href']
